...
public class Day28 {
    public int[] maxPoints(int[][] grid, int[] queries) {
        int[][] require = new int[grid.length][grid[0].length];
        require[0][0] = grid[0][0];
        for (int i = 1; i < grid[0].length; i++) {
            int cur = grid[0][i];
            int prev = require[0][i - 1];
            require[0][i] = Math.max(prev, cur);
        }
        for (int i = 1; i < grid.length; i++) {
            int cur = grid[i][0];
            int prev = require[i - 1][0];
            require[i][0] = Math.max(prev, cur);
        }
        for (int i = 1; i < grid.length; i++) {
            for (int j = 1; j < grid[0].length; j++) {
                int up = require[i - 1][j];
                int left = require[i][j - 1];
                require[i][j] = Math.max(grid[i][j], Math.min(up, left));
            }
        }
        System.out.println(Arrays.deepToString(require));
        int[] result = new int[queries.length];
        for (int i = 0; i < queries.length; i++) {
            result[i] = this.count(require, queries[i]);
        }
        return result;
    }

    private int count(int[][] require, int query) {
        int count = 0;
        for (int[] ints : require) {
            for (int anInt : ints) {
                if (anInt < query) count++;
            }
        }
        return count;
    }
    ...
    public static void main(String[] args) {
        ...
        int[] ints = day28.maxPoints(new int[][]{
        ...
        }, new int[]{690474, 796832, 913701, 939418, 46696, 266869, 150594, 948153, 718874});
